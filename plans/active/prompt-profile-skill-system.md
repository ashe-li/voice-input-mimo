...
| `Sources/VoiceInputMimo/Prompts/PromptStoreProviding.swift` | Phase 4 強制 — protocol + `PromptStore: PromptStoreProviding` extension（DI 注入用）|
| `Sources/VoiceInputMimo/Refining.swift` | Phase 4 強制 — `Refining` protocol + `LLMRefiner: Refining` extension |
...
### MainActor isolation（Phase 2-5 強制）

所有 `NSViewController` / `NSView` 子類必須 `@MainActor` 標記：

```swift
@MainActor
final class ProfileEditor: NSViewController {
    private let store: any PromptStoreProviding

    override func viewDidLoad() {
        super.viewDidLoad()
        Task {
            // PromptStore 是 final class + DispatchQueue（v1）；v1.5 升 actor 後改 await
            let profile = try? store.activeProfile(for: .refine)
            updateUI(with: profile)  // back on MainActor 已隱含
        }
    }
}
```

Swift 6.2 inference mode 會在跨 actor 呼叫漏標時直接 compile error；不要靠 `DispatchQueue.main.async` 補救（那只是 runtime 對齊、不解 isolation）。

### Protocol-based DI（Phase 4 強制）

Phase 4 三欄 UI（ProfileSidebar / ProfileEditor / PromptTestPanel）不能直綁 `PromptStore.shared` / `LLMRefiner.shared`，必須吃 protocol，預設 init 帶 `.shared` 單例、測試注 mock：
...
@MainActor
final class PromptTestPanel: NSViewController {
...
- `Sources/VoiceInputMimo/LLMRefiner.swift` 加 `Refining` conformance
...
| 2（UI 共用元件）| `swift build` warning-free + 單元 snapshot 各元件 | `phase2_gate.sh` — build + unit + 元件可實例化驗 |
...
| MainActor 邊界違反（Swift 6.2 compile error）| 所有 `NSViewController` / `NSView` 子類必須 `@MainActor`；跨 actor 呼叫一律 `Task { ... await ... }`，不能靠 `DispatchQueue.main.async` 補救 |
| 直接綁 `.shared` 單例導致 UI 無法測試 | Phase 4 起 ProfileEditor / PromptTestPanel 必須吃 `PromptStoreProviding` / `Refining` protocol，預設 init 帶 singleton、測試注 mock |
...
> **Phase 2 開工前必做**：ECC plugin 的 4 個 Swift skill 預設 deferred（`swiftui-patterns` / `swift-concurrency-6-2` / `swift-protocol-di-testing` / `swift-actor-persistence`），已用 Explore agent 讀過 4 個 skill 的內容並把規約整合進「## Concurrency / DI / MainActor 規約（Phase 2+）」與「## UI Stack Decision（v1：純 AppKit）」兩段落（plan 內對齊完成）。Phase 2 開工流程：
...
> 2. 寫 NSViewController 子類前 `ToolSearch query="swift concurrency"` 拉 deferred skill 進 context（驗證細節）
> 3. Code review 走 generic `code-reviewer` agent，但 prompt 須點名 Swift idioms 檢查項：optional unwrap、`@MainActor` 邊界、`Sendable`、retain cycle、closure `[weak self]`
> 4. ECC 沒有 `swift-build-resolver` agent — `swift build` 失敗時人工讀 stderr，不要套 generic build-error-resolver（取向 npm/TS）
...
## UI Stack Decision（v1：純 AppKit）

來源：ECC `swiftui-patterns` skill 對齊。

**v1 鎖定純 AppKit**（NSViewController / NSCollectionView / NSPanel）—— 不混 SwiftUI：

- 既有 codebase（`AppDelegate` / `OverlayPanel` / `ClipboardHistoryWindow` / `SettingsWindow` / `ModelMemoryWindow`）全 AppKit、LSUIElement menubar app
- 引入 SwiftUI 需重新設計 state management（@Observable / EnvironmentValues / SceneStorage），與 menubar app lifecycle 整合度差
- Phase 2 五個共用元件（VibrantWindow / SectionHeading / RoundedCard / SidebarList / IconButton）皆為 NSView / NSViewController 子類

**v1.5 可選升級**：若 SkillsLibraryView 演化成複雜 list，可單獨換成 SwiftUI `List` + `@Observable` store + `.environment(...)` 注入；但**不能 mix 一個 pane**——SwiftUI / AppKit 邊界以整個 pane 為單位。

**SwiftUI 引入時的紀律**（v1.5+）：
- state 用 `@Observable`（macOS 14+）不要 `ObservableObject`
- store 用 `.environment(store)` modifier 注入，不要 singleton 抓取
- view 預設 `@MainActor`（Swift 6.2 inference）

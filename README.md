...
> - 新 engine 在 sibling repo [`voice-input-mimo-asr-engine/`](../voice-input-mimo-asr-engine)，由 Swift app 預設啟動於 port 8766
...
└── Sources/, Package.swift, Makefile         ← Phase B Swift app（已完成）
```

Phase 2 engine（adaptive idle ladder 3/7/15 min + Qwen cache poll manager + 三層結構化 log + X-Request-Id end-to-end correlation）住在 sibling repo `~/Documents/voice-input-mimo-asr-engine/`，Swift app `LocalASRServer.swift` 預設指向那裡 + port 8766。
...
- `ASRClient.swift`：multipart POST → `/v1/audio/transcriptions`，X-Request-Id 自動 forward
...
- `KeyMonitor.swift`：CGEventTap 監 Fn key，down → 開錄音、up → 停 → POST → refine → 貼上
- `SettingsWindow.swift`：可改 dir / port / module / 自動 5s poll engine 狀態 / Probe 走 smoke transcribe
- bundle id `com.shiun.VoiceInputMimo` 跟 Apple Speech 版（`voice-input-src`）並存

`make install` 建置 + 安裝到 `/Applications/`。Phase 2 engine wire-up 切到 8766 / `engine.server:app` 走 UserDefaults 一次性 migration。
